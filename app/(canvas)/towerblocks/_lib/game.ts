...
export const setupTowerBlocks = (
  ...
  const dropBlock = () => {
    ...
    if (isPerfect) {
      // Perfect placement: match the top block position/width
      comboCount++;
  ...
  const updateCamera = () => {
    ...
    // Clamp: don't scroll below ground
  ...
  const drawSkyGradient = (heightRatio: number) => {
    ...
    // Pastel sky: transitions from soft blue to warm peach/pink as height increases
    // Top color: pastel blue → deeper sky blue
    const r1 = Math.floor(160 - heightRatio * 40);
    const g1 = Math.floor(200 + heightRatio * 30);
    const b1 = Math.floor(235 + heightRatio * 20);

    // Bottom color: warm peach → soft lavender
