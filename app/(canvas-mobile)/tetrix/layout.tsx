...
export const metadata = getGameMetadata('/tetrix');
const jsonLd = getGameJsonLd('/tetrix');

function TetrisLayout({ children }: { children: React.ReactNode }) {
  return (
    <main className="w-full h-screen px-4 xl:px-6 py-4 flex flex-col gap-4 xl:gap-10 items-center">
      {jsonLd && <JsonLd data={jsonLd} />}
      <KameHeader title="Tetrix" />
