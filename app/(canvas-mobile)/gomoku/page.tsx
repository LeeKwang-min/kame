...
const controls = [
  ...
  { key: 'P', action: '일시정지' },
  { key: 'R', action: '재시작' },
  { key: '1-4', action: '난이도 선택' },
];

function GomokuPage() {
  const { data: session, status } = useSession();
  const { data: scores = [], isLoading } = useGetScores('gomoku' as Parameters<typeof useGetScores>[0]);
