...
import { GAME_TYPE, MAX_PLAYERS } from '../_lib/config';
...
function GomokuLobby() {
  ...
  useEffect(() => {
    const offCreated = on('room:created', (data: { room: TRoomDetail }) => {
      setIsCreating(false);
      router.push(`/gomoku-online/${data.room.id}`);
    });
  ...
  const handleCreateRoom = useCallback(() => {
    ...
    setIsCreating(true);
    sessionStorage.setItem('gomoku:playerName', effectivePlayerName);
    emit('room:create', {
      gameType: GAME_TYPE,
  ...
  const handleJoinRoom = useCallback(
    (roomId: string) => {
      if (!isLoggedIn) {
        signIn('google', { callbackUrl: '/gomoku-online' });
        return;
      }
      if (!effectivePlayerName) return;
      sessionStorage.setItem('gomoku:playerName', effectivePlayerName);
    ...
    <div className="w-full max-w-2xl mx-auto flex flex-col gap-6">
      ...
      {isLoggedIn ? (
        ...
        <div className="flex items-center justify-between p-4 bg-arcade-surface border border-arcade-border rounded-lg">
          ...
          <button
            onClick={() => signIn('google', { callbackUrl: '/gomoku-online' })}
