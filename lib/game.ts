...
export const gameStartHud = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
  const rect = canvas.getBoundingClientRect();
  ...
  ctx.fillText("Using Space and Arrow keys", rect.width / 2, rect.height / 2 + 60);
  ctx.restore();
}

export const gameHud = (ctx: CanvasRenderingContext2D, score: number, sec: number) => {
  ctx.save();
  const time = sec;
  const totalScore = Math.floor(score);
...
export const gameOverHud = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, score: number) => {
  const rect = canvas.getBoundingClientRect();
  const totalScore = Math.floor(score);
...
export const drawHud = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, score: number, sec: number, isStarted: boolean, isGameOver: boolean) => {
  if (!isStarted) {
